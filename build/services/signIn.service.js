...
const signInWithCredentials = (statusCode, res, req, next, user) =>
  __awaiter(void 0, void 0, void 0, function* () {
    // generate unique access token when user logs in
    const accessToken = jsonwebtoken_1.default.sign(
      { user },
      process.env.SIGN_IN_ACCESS_SECRET_KEY,
      { expiresIn: `${process.env.ACCESS_TOKEN_EXPIRATION}m` || "30m" }
    );
    // generate unique refresh token when user logs in
    const refreshToken = jsonwebtoken_1.default.sign(
      { user },
      process.env.SIGN_IN_REFRESH_SECRET_KEY,
      { expiresIn: `${process.env.REFRESH_TOKEN_EXPIRATION}d` || "5d" }
    );
    // accessToken expires in
    const accessTokenExpiresAt = new Date(
      Date.now() + token_1.accessTokenOptions.maxAge
    ).getTime();
    ...
    if (
      (user === null || user === void 0 ? void 0 : user.signedInAs) === "user"
    ) {
      ...
      (user === null || user === void 0 ? void 0 : user.signedInAs) === "doctor"
    ) {
